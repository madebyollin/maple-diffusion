...
func loadConstant(graph: MPSGraph, name: String, shape: [NSNumber], fp32: Bool = false) -> MPSGraphTensor {
    let numels = shape.map(\.intValue).reduce(1, *)
...
func makeUpsampleNearest(graph: MPSGraph, xIn: MPSGraphTensor, scaleFactor: Int = 2) -> MPSGraphTensor {
    graph.resize(xIn, size: [NSNumber(value: xIn.shape![1].intValue * scaleFactor), NSNumber(value: xIn.shape![2].intValue * scaleFactor)], mode: MPSGraphResizeMode.nearest, centerResult: true, alignCorners: false, layout: MPSGraphTensorNamedDataLayout.NHWC, name: nil)
}
...
func makeSwish(graph: MPSGraph, xIn: MPSGraphTensor) -> MPSGraphTensor {
    graph.multiplication(xIn, graph.sigmoid(with: xIn, name: nil), name: nil)
}

func makeGroupNormSwish(graph: MPSGraph, xIn: MPSGraphTensor, name: String) -> MPSGraphTensor {
    makeSwish(graph: graph, xIn: makeGroupNorm(graph: graph, xIn: xIn, name: name))
}
...
func makeDecoder(graph: MPSGraph, xIn: MPSGraphTensor) -> MPSGraphTensor {
    ...
    for i: Int in 0...3 {
        let l: Int = 3 - i
        let k: NSNumber = (l > 1) ? 512 : .init(value: 128 * (l + 1))

        for j: Int in 0...2 {
            x = makeDecoderResBlock(graph: graph, xIn: x, name: name + ".up.\(l).block.\(j)", outChannels: k)
        }

        if l > 0 {
            x = makeUpsampleNearest(graph: graph, xIn: x)
            x = makeConv(graph: graph, xIn: x, name: name + ".up.\(l).upsample.conv", outChannels: k, khw: 3)
        }
    }
...
func makeUNetAnUnexpectedJourney(graph: MPSGraph, xIn: MPSGraphTensor, tembIn: MPSGraphTensor, condIn: MPSGraphTensor, name: String, saveMemory: Bool = true) -> [MPSGraphTensor] {
    ...
    for i: Int in 1...9 {
        ...
        if i % 3 == 0 {
            x = makeConv(graph: graph, xIn: x, name: name + ".input_blocks.\(i).0.op", outChannels: j, khw: 3, stride: 2)
        } else {
            x = makeUNetResBlock(graph: graph, xIn: x, embIn: emb, name: name + ".input_blocks.\(i).0", inChannels: j, outChannels: k)
            x = makeSpatialTransformerBlock(graph: graph, xIn: x, name: name + ".input_blocks.\(i).1", contextIn: condIn, saveMemory: saveMemory)
        }
...
func makeUNetTheDesolationOfSmaug(graph: MPSGraph, savedInputsIn: [MPSGraphTensor], name: String, saveMemory: Bool = true) -> [MPSGraphTensor] {
    ...
    for i: Int in 0...4 {
        ...
        x = makeOutputBlock(graph: graph, xIn: x, embIn: emb, condIn: condIn, inChannels: 2560, outChannels: 1280, dHead: 160, name: name + ".output_blocks.\(i)", saveMemory: saveMemory, spatialTransformer: j, upsample: k)
    }
...
func makeUNetTheBattleOfTheFiveArmies(graph: MPSGraph, savedInputsIn: [MPSGraphTensor], name: String, saveMemory: Bool = true) -> MPSGraphTensor {
    ...
    for i: Int in 5...11 {
        ...
        x = graph.concatTensors([x, savedInputs.popLast()!], dimension: 3, name: nil)
        x = makeOutputBlock(graph: graph, xIn: x, embIn: emb, condIn: condIn, inChannels: j, outChannels: k, dHead: l, name: name + ".output_blocks.\(i)", saveMemory: saveMemory, spatialTransformer: true, upsample: m)
    }
...
func makeSqrtOneMinus(graph: MPSGraph, xIn: MPSGraphTensor) -> MPSGraphTensor {
    graph.squareRoot(with: graph.subtraction(graph.constant(1.0, dataType: MPSDataType.float16), xIn, name: nil), name: nil)
}
...
class BPETokenizer {
    ...
    func whitespaceClean(s: String) -> String { s.components(separatedBy: .whitespacesAndNewlines).filter { !$0.isEmpty }.joined(separator: " ").trimmingCharacters(in: .whitespacesAndNewlines) }
    
    func getPairs(s: [String]) -> Set<String> { Set<String>((1..<s.count).map { s[$0 - 1] + " " + s[$0] }) }
    ...
    func encodeToken(s: String) -> [Int] {
        ...
        if !pairs.isEmpty {
            while true {
                ...
                if ranks[highestRankedBigram] == nil {
                    break
                }
