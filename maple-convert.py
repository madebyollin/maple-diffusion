...
if len(sys.argv) < 2:
    raise ValueError(f"Usage: {sys.argv[0]} path_to_ckpt")
...
if not vocab_dest.exists():
    ...
    with requests.get(vocab_url, stream=True) as r:
        assert (
            r.status_code == 200
        ), f"{vocab_url} failed to download. please copy it to {vocab_dest} manually."
        with vocab_dest.open("wb") as vf:
            for c in r.iter_content(chunk_size=8192):
                vf.write(c)
        print("downloaded clip vocab")

# add support for model without a key "state_dict"
...
# other stuff
th.exp(-th.log(th.tensor([10000])) * th.arange(0, 160) / 160).numpy().tofile(
    outpath / "temb_coefficients_fp32.bin"
)
np.triu(np.ones((1, 1, 77, 77), dtype=np.float16) * -65500.0, k=1).astype(
    np.float16
).tofile(outpath / "causal_mask.bin")
np.array(
    [
        0.14013671875,
        0.0711669921875,
...
).astype(np.float16).tofile(outpath / "aux_output_conv.weight.bin")
np.array([0.423828125, 0.471923828125, 0.473876953125]).astype(np.float16).tofile(
    outpath / "aux_output_conv.bias.bin"
)
print(f"Done!")
